...
Once the `ramonaoptics` channel has been enabled, `python` can be installed with `conda`:
...
It is possible to list all of the versions of `python` available on your platform with `conda`:
...
```
mamba search python --channel ramonaoptics
```
...
mamba repoquery search python --channel ramonaoptics

# List packages depending on `python`:
mamba repoquery whoneeds python --channel ramonaoptics

# List dependencies of `python`:
mamba repoquery depends python --channel ramonaoptics
```
